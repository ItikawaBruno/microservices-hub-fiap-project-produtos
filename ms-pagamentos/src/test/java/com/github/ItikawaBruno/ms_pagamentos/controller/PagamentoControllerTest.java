...
public class PagamentoControllerTest {
    ...
    void findAllPagamentosShouldReturnListPagamentoDTO() throws Exception {
        ...
        Mockito.when(pagamentoService.listPagamentos()).thenReturn(list);

        ResultActions result = mockMvc.perform(get("/pagamentos").accept(String.valueOf(MediaType.APPLICATION_JSON)));

        result.andDo(print());
        result.andExpect(status().isOk());
        result.andExpect(content().contentTypeCompatibleWith(String.valueOf(MediaType.APPLICATION_JSON)));
        ...
        Mockito.verify(pagamentoService).listPagamentos();
    ...
    void createPagamentoShouldReturn201WhenValid() throws Exception{
        ...
        Mockito.when(pagamentoService.createPagamento(any(PagamentoDTO.class))).thenReturn(responseDTO);
        ...
        mockMvc.perform(post("/pagamentos")
                ...
                .andExpect(jsonPath("$.staus").value(pagamento.getStatus().name()))
                .andExpect(jsonPath("$.valor").value(pagamento.getValor().doubleValue()))
                .andExpect(jsonPath("$.pedidoId").value(pagamento.getPedidoId()));

        Mockito.verify(pagamentoService).createPagamento(any(PagamentoDTO.class));
        Mockito.verifyNoMoreInteractions(pagamentoService);
    ...
    void deletePagamentoShouldReturn204WhenIdExists() throws Exception {
        ...
        mockMvc.perform(delete("/pagamentos/{id}", existingId)).andExpect(status().isNoContent());
    ...
    @Test
    void deletePagamentosShouldReturn404WhenIdDoesNotExist() throws Exception {
    ...
    @Test
    void createPagametnoShouldReturn422WhenInvalid() throws Exception{
